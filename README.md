...
<br>Скрипт предназначен для перезапуска контейнера с AWG в случае если потеряно соединение. </br>
<br>Принцип работы: </br>
<br>Скрипт запускается на сервере, где развернут контейнер с впн. </br>
<br>По CRON проверяет "живой ли" handshake у awg до любого из клиентов. </br>
<br>Если работа корректна, то ничего не происходит. </br>
<br>Иначе запускается цикл рестартов с сохранением логов и отправкой в ТГ сообщения. </br>
